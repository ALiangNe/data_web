...
import { getChatHistories } from '@/api/data'
...
const table = DATA_CENTER_TABLES.chatHistories
...
const sortField = ref<DataCenterSortFieldFor<'chatHistories'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    try {
        ...
        console.error('ChatHistoriesView fetch failed:', error)
        rows.value = []
        total.value = 0
...
const onSortColumn = (col: string) => {
    ...
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'chatHistories'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'chatHistories'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
