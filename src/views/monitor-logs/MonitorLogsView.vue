...
const fetchData = async () => {
    ...
    for (const [field, range] of Object.entries(timeRangeValues.value)) {
        ...
        timeRangeFilters[field] = [
            start ? (field === 'startTimeMs' ? String(new Date(start).getTime()) : new Date(start).toISOString()) : '',
            end ? (field === 'startTimeMs' ? String(new Date(end).getTime()) : new Date(end).toISOString()) : '',
        ]
    }
