<template>
    <div class="data-list-view">
        ...
        <DataTable :columns="columns" :rows="rows" :sortable-fields="sortableFields" :sort-field="sortField"
            :sort-order="sortOrder" :loading="loading" @sort-column="onSortColumn" />
...
const table = DATA_CENTER_TABLES.media
...
const sortField = ref<DataCenterSortFieldFor<'media'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    for (const [key, rawValue] of Object.entries(filterValues.value)) {
        ...
        if (key === 'sizeBytes' && Number.isFinite(Number(v))) filters[key] = Number(v)
        else filters[key] = v
    }
    ...
    try {
        ...
        console.error('MediaView fetch failed:', error)
        rows.value = []
        total.value = 0
...
const onSortColumn = (col: string) => {
    ...
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'media'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'media'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
