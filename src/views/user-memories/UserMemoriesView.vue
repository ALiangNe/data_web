...
import { getUserMemories } from '@/api/data'
...
const table = DATA_CENTER_TABLES.userMemories
...
const sortField = ref<DataCenterSortFieldFor<'userMemories'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    try {
        ...
        console.error('UserMemoriesView fetch failed:', error)
        rows.value = []
        total.value = 0
...
const onSortColumn = (col: string) => {
    ...
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'userMemories'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'userMemories'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
