...
import { getAuthProviders } from '@/api/data'
...
const table = DATA_CENTER_TABLES.authProviders
...
const sortField = ref<DataCenterSortFieldFor<'authProviders'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    for (const [key, rawValue] of Object.entries(filterValues.value)) {
        ...
        if (key === 'emailVerified' && (v === 'true' || v === '1')) filters[key] = true
        else if (key === 'emailVerified' && (v === 'false' || v === '0')) filters[key] = false
        else filters[key] = v
    }
    ...
    try {
        ...
        console.error('AuthProvidersView fetch failed:', error)
        rows.value = []
        total.value = 0
...
const onSortColumn = (col: string) => {
    ...
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'authProviders'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'authProviders'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
