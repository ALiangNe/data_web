...
import { getChatTopics } from '@/api/data'
...
const table = DATA_CENTER_TABLES.chatTopics
...
const sortField = ref<DataCenterSortFieldFor<'chatTopics'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    try {
        ...
        console.error('ChatTopicsView fetch failed:', error)
        rows.value = []
        total.value = 0
    ...
    rows.value = data.list.map((row) => {
        ...
        for (const [key, value] of Object.entries(row)) {
            ...
            else if (key === 'createdAt' || key === 'updatedAt' || key === 'startedAt' || key === 'endedAt') formatted[key] = new Date(value as string).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })
            else if (key === 'embedding') formatted[key] = `Array[${(value as number[]).length}]`
            else if (key === 'participants') formatted[key] = `{${(value as string[]).join(',')}}`
...
const onSortColumn = (col: string) => {
    ...
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'chatTopics'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'chatTopics'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
