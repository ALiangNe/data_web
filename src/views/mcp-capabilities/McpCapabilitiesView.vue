...
import { getMcpCapabilities } from '@/api/data'
...
const table = DATA_CENTER_TABLES.mcpCapabilities
const pageSizeOptions = [5, 10, 20]
const filterFields = table.filter
const sortableFields = table.sortFields
const filterValues = ref<Record<string, string>>({})
const sortField = ref<DataCenterSortFieldFor<'mcpCapabilities'>>(table.sortFields[0])
...
const fetchData = async () => {
    ...
    try {
        ...
        console.error('McpCapabilitiesView fetch failed:', error)
        rows.value = []
        total.value = 0
...
const onSortColumn = (col: string) => {
    if (loading.value) return
    if (!sortableFields.includes(col as DataCenterSortFieldFor<'mcpCapabilities'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataCenterSortFieldFor<'mcpCapabilities'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
