...
import { getMcpCapabilities } from '@/api/resource'
...
const table = DATA_TABLES.mcpCapabilities
const pageSizeOptions = [5, 10, 20]
const filterFields = table.filter
const sortableFields = table.sortFields
const filterValues = ref<Record<string, string>>({})
const sortField = ref<DataSortFieldFor<'mcpCapabilities'>>(table.sortFields[0])
...
const onSortColumn = (col: string) => {
    if (loading.value) return
    if (!sortableFields.includes(col as DataSortFieldFor<'mcpCapabilities'>)) return
    if (col === sortField.value) {
        sortOrder.value = sortOrder.value === 'desc' ? 'asc' : 'desc'
    } else {
        sortField.value = col as DataSortFieldFor<'mcpCapabilities'>
        sortOrder.value = 'desc'
    }
    page.value = 1
    fetchData()
